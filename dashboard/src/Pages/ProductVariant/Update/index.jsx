...
const UpdateProductVariant = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const response = await fetchData(`product-variant/${id}?populate=productId,variantId`, {
          method: "GET",
          headers: { authorization: `Bearer ${token}` },
        });
        if (response.success) {
  ...
  return (
    <div className="max-w-xl mx-auto bg-white p-6 rounded-lg shadow-md">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Variant Selection - Updated for new model */}
        <div>
          <label className="block text-sm font-medium text-gray-700">Variant (Color - Size) *</label>
          <select
            name="variantId"
            value={getFieldValue("variantId")}
            onChange={handleChange}
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
            required
          >
